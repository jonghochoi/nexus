...
Usage:
    ...
    python upload_eval.py \\
        --eval_dir   ./eval_results/exp1__ckpt_5000__20260510 \\
        --run_info   ./logs/exp1/run_001/PPO
    ...
    python upload_eval.py \\
        ...
        --run_name     exp1_run_001 \\
        --metrics_from ./eval_results/exp1__ckpt_5000__20260510/metrics.json \\
...
Workflow context:
  ...
  The CLI defaults to ``target=central`` when ``--run_info`` is provided
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Sidecar URI to use when --run_info is provided (default: central). "
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="MLflow server URI. Required when --run_info is not provided.",
    )
    parser.add_argument(
        "--experiment",
        type=str,
        default=None,
        help="Experiment name owning the run. Required when --run_info is not provided.",
    )
    parser.add_argument(
        "--run_name",
        type=str,
        default=None,
        help="Run name to attach artifacts to. Required when --run_info is not provided.",
    )
...
def main() -> int:
    ...
    if args.eval_dir is None:
        console.print("[red]Missing required:[/red] --eval_dir")
        return 1
    ...
    if args.run_info is not None:
        try:
            ...
            console.print(f"[red]--run_info resolution failed:[/red] {e}")
            return 1
    else:
        missing = [
            name
            for name in ("tracking_uri", "experiment", "run_name")
            if getattr(args, name) is None
        ]
        if missing:
            console.print(
                f"[red]Missing required:[/red] {', '.join('--' + m for m in missing)} "
                "(or pass --run_info to read identity from .nexus_run.json)"
            )
            return 1
        ev = EvalLogger(
            run_name=args.run_name, tracking_uri=args.tracking_uri, experiment=args.experiment
        )
