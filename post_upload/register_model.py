...
def parse_args(defaults: dict):
    ...
    parser.add_argument(
        ...
        help=f"MLflow server URI — typically central (default: {defaults['tracking_uri']})",
    )
    parser.add_argument(
        "--experiment",
        type=str,
        default=defaults["experiment"],
        help=f"Experiment name the source run lives in (default: {defaults['experiment']})",
    )
    parser.add_argument(
        "--run_name", type=str, required=True, help="Source run name (run identity)"
    )
    parser.add_argument(
        "--model_name", type=str, required=True, help="Model Registry name to register against"
    )
    parser.add_argument(
        "--kind",
        type=str,
...
def main() -> int:
    ...
    args = parse_args({"tracking_uri": config["tracking_uri"], "experiment": config["experiment"]})
