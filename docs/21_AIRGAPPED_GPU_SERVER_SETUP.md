...
**Export those values as shell variables on your local machine** so every command below stays in sync:

```bash
# Example values — replace with whatever the GPU server reported above.
export GPU_PY=3.12                         # major.minor only (no patch)
export GPU_PLATFORM=manylinux2014_x86_64   # x86_64 → manylinux2014_x86_64
```

**Download the wheels for that target:**
...
# Install virtualenv with system pip first (alternative to venv module)
pip install --no-index --find-links ~/nexus_wheels --break-system-packages virtualenv

# Create virtual environment at ~/.nexus/venv (outside the repo so
...
# Install pinned setuptools version (versions 70+ exclude pkg_resources)
