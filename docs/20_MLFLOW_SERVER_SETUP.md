...
- [Step 9 — Enable daily backups *(optional)*](#step-9--enable-daily-backups-optional)
...
Step 9    Enable daily backups                       (optional, recommended)
```
...
>
> With `--default-artifact-root <local-path>` (the old style), MLflow **clients** (e.g. the GPU server running `mlflow.log_artifacts()` or `upload_tb.py --upload-artifacts`) try to write directly to that local filesystem path. Remote clients have no access to `/opt/nexus-mlflow/artifacts` on the server, so the upload fails with `Permission denied`.
>
...
## Step 9 — Enable daily backups *(optional)*

> [!NOTE]
> This step is **optional** — the server runs fine without it, and you can return to it later. It is **strongly recommended** for any team with more than one researcher or any data you would not want to recreate from scratch.
...
For the full backup/restore guide — including off-site replication, the disaster-recovery runbook, and the quarterly drill — see **[`22_BACKUP.md`](22_BACKUP.md)**.
...
[Spare PC — NEXUS MLflow Server]
  ...
  Backup tree (only present if Step 9 was enabled — optional):
...
[Daily backup]                            (optional — only if Step 9 was enabled)
  Cron:    /etc/cron.d/nexus-mlflow-backup (03:00, runs as nexus-mlflow)
  Script:  /opt/nexus/scheduled_sync/backup_mlflow.sh
  Restore: /opt/nexus/scheduled_sync/restore_mlflow.sh (see 22_BACKUP.md)
```
...
- **Set up daily backups of the central tree** *(optional, recommended)* → [`22_BACKUP.md`](22_BACKUP.md)
