...
For shell-driven workflows (post-eval glue scripts, CI jobs, ad-hoc uploads from the operator desktop), `post_upload/upload_eval.py` wraps the same API. Sidecar-driven invocation matches the Python quick start one-for-one — pass the trainer's `tb_dir` to `--run_info` and the eval bundle to `--eval_dir`:
...
`--dry_run` resolves the run and lists what would be uploaded without touching MLflow. `--history` prints recent `upload_eval` invocations (recorded in `~/.nexus/history.json` alongside `upload_tb` / `register_model`).
